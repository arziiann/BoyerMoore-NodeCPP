...
std::vector<int> goodSuffixHeuristic(const std::string& pattern, int badchar[NO_OF_CHARS]);

void badCharacterHeuristic(const std::string& pattern, int badCharTable[256]) {
    int patternLength = pattern.size();

    // Սկզբում նշում ենք, որ բոլոր սիմվոլները pattern-ում չկան
    std::fill(badCharTable, badCharTable + 256, -1);

    // Յուրաքանչյուր սիմվոլի վերջին հանդիպման դիրքը pattern-ում
